...
Project-level Codebase Awareness caches under `.signum/cache/`, including `file-digests-v1.json` and `file-extracts-v1.json`, are rebuildable scanner cache, not active contract root evidence and not proofpack payloads. The digest cache supports bounded/incremental lexical scanning, and the extraction cache stores per-file scanner extraction payloads for unchanged indexed files when path, SHA-256, size, schema version, scan mode, and extractor version are compatible. Neither cache adds AST, semantic, or external-tool scanning. Scanner defaults are bounded at `--max-files 10000`, `--max-bytes 50000000`, and `--max-file-size 1048576`.
