...
`.signum/cache/file-extracts-v1.json` is a rebuildable project-level Codebase Awareness cache. It stores per-file scanner extraction payloads for unchanged indexed files and may be reused with `--previous-extracts` when the repo-relative path, SHA-256, size, schema version, scan mode, and extractor version are compatible. It is not active contract root evidence, not run-scoped evidence, and not a proofpack payload. The cache is derived from the same shallow stdlib scanner and does not add AST, semantic, or external-tool scanning.
