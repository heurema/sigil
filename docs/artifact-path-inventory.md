...
Root `.signum/*` artifact files are compatibility or legacy migration inputs only. Root `.signum/contracts/`, `.signum/archive/`, project-level indexes, policy, metrics, derived cache, and session files are registry/state surfaces, not per-run working-set artifact roots.
