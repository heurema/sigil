...
Root `.signum/*` artifact files are compatibility or legacy migration inputs only. Root `.signum/contracts/`, `.signum/archive/`, project-level indexes, policy, metrics, and session files are registry/state surfaces, not per-run working-set artifact roots.
...
| Project-level state, policy, and metrics | `lib/proofpack-index.sh`, `lib/metric-ratchet.sh`, `lib/pack-anti-entropy.sh`, `lib/policy-resolver.sh`, `lib/session-manager.sh` | Uses `.signum/proofpack-index.jsonl`, `.signum/metrics/ratchet-report.json`, `.signum/policy.toml`, and `.signum/session.json`. These are project-level state/config surfaces, not per-run root artifacts. | Keep; do not mix with per-run artifact migration. |
