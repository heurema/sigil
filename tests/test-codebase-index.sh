...
echo "=== Scanner run ==="
if run_scanner ".signum/run-a/cache/codebase-index-v1.json" ".signum/run-a/cache/style-profile-v1.json"; then
...
if python3 - "$INDEX_A" "$STYLE_A" "$PROJECT" <<'PY'
import json
import sys
...
if project in json.dumps(index) or project in json.dumps(style):
...
echo ""
echo "=== Determinism ==="
if run_scanner ".signum/run-b/cache/codebase-index-v1.json" ".signum/run-b/cache/style-profile-v1.json"; then
