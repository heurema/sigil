...
check_file() {
  ...
  assert_contains "$bootstrap" 'keep only the minimal root compatibility set eagerly materialized' "$label bootstrap"
  ...
  assert_contains "$bootstrap" 'ensure_active_artifact_dir "reviews"' "$label bootstrap"
