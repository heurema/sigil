...
check_file() {
  ...
  assert_contains "$bootstrap" 'keep only the root contract compatibility path eagerly materialized' "$label bootstrap"
  assert_contains "$bootstrap" 'lazy/on-demand' "$label bootstrap"
