...
check_file() {
  ...
  assert_not_contains "$launch" 'link_active_artifact "contract.json"' "$label launch"
  ...
  assert_contains "$bootstrap" 'Normal runs must not materialize root `.signum/` artifact views' "$label bootstrap"
  assert_contains "$bootstrap" 'mkdir -p "${ARTIFACT_ROOT}reviews"' "$label bootstrap"
  assert_not_contains "$bootstrap" 'link_active_artifact "contract.json"' "$label bootstrap"
  assert_not_contains "$bootstrap" 'ensure_active_artifact_dir "reviews"' "$label bootstrap"
  assert_not_contains "$bootstrap" 'link_active_artifact "spec_quality.json"' "$label bootstrap"
  assert_not_contains "$bootstrap" 'link_active_artifact "review_context.json"' "$label bootstrap"
  assert_not_contains "$bootstrap" 'link_active_artifact "review_prompt_codex.txt"' "$label bootstrap"
  assert_not_contains "$bootstrap" 'ensure_active_artifact_dir "receipts"' "$label bootstrap"
